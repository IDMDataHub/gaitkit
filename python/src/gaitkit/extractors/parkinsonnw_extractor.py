"""
Parkinson Nordic Walking Extractor -- PD Gait Dataset (Nordic Walking intervention).

Dataset: Parkinson Motion Dataset - Nordic Walking Group
Source: Figshare 29371769
Content:
- 144 gait trials (24 subjects × Pre/Post × 3 trials)
- PiG markers (178 channels), 100 Hz
- 2 force plates
- Events: Foot Strike / Foot Off with Left/Right CONTEXTS (~13 per trial)

Structure:
  dataset online/ParkinsonMotionDataset/Dataset/Group_Study_NordicWalking/
    ID{XX}/ID{XX}_Pre/ID{XX}_Pre_T1/ID{XX}_Pre_T1_c3d.c3d
    ID{XX}/ID{XX}_Pre/ID{XX}_Pre_T2/...
    ID{XX}/ID{XX}_Post/ID{XX}_Post_T1/ID{XX}_Post_T1_c3d.c3d
  (Exclude _S_ = static calibrations)
...
class ParkinsonNWExtractor(BaseExtractor):
    """Extractor for Parkinson Nordic Walking dataset (PiG markers)."""
    ...
    @property
    def name(self) -> str:
        return "Parkinson Nordic Walking"

    @property
    def description(self) -> str:
        return "Parkinson disease: 144 trials, 24 subjects, pre/post Nordic Walking, 100 Hz PiG"

    def list_files(self) -> List[Path]:
        if not HAS_EZC3D:
            return []
        files = list(self.data_dir.rglob('*.c3d'))
        # Exclude static calibrations: filenames containing _S_ (e.g. ID01_Pre_S_c3d.c3d)
        files = [f for f in files
                 if '_S_' not in f.stem
    ...
    def _parse_filename(self, filepath: Path) -> Dict:
        ...
        Format: ID{XX}_{Pre|Post}_T{N}_c3d.c3d
        e.g. ID01_Pre_T1_c3d.c3d
        """
        info = {
            'subject_id': 'unknown',
            'condition': 'unknown',
        ...
        filename = filepath.stem  # e.g. ID01_Pre_T1_c3d

        m = re.match(r'(ID\d+)_(Pre|Post)_T(\d+)', filename, re.IGNORECASE)
        if m:
            info['subject_id'] = m.group(1).upper()
            info['condition'] = m.group(2).lower()  # pre or post
            info['trial_id'] = f'T{m.group(3)}'
