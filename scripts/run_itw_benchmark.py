#!/usr/bin/env python3
"""
ITW Benchmark — Run all gaitkit detectors on the ITW dataset.

Usage (on server):
    source ~/gait_venv/bin/activate
    python ~/gaitkit/scripts/run_itw_benchmark.py \
        --data-dir ~/gait_benchmark_project/data/ITW \
        --output   ~/gait_benchmark_project/results/benchmark_itw.csv
...
import gaitkit
from gaitkit.extractors.itw_extractor import ITWExtractor
...
def run_benchmark(data_dir, methods, tolerances):
    ext = ITWExtractor(data_dir)
    files = ext.list_files()
    print(f"ITW: found {len(files)} trials")
    ...
    for fi, fpath in enumerate(files):
        ...
        # Determine population from filepath
        file_info = ext._parse_filepath(fpath)
        population = file_info.get('population', 'itw')
...
def main():
    parser = argparse.ArgumentParser(description="Run BIKE benchmark on ITW dataset")
    ...
    print("=" * 70)
    print("ITW BENCHMARK (gaitkit detectors)")
    print("=" * 70)
    ...
    if args.output:
        ...
        out_path = Path("results") / f"benchmark_itw_{ts}.csv"
