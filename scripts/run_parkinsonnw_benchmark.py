#!/usr/bin/env python3
"""
Parkinson NW Benchmark — Run all gaitkit detectors on the Parkinson Nordic Walking dataset.

Usage (on server):
    source ~/gait_venv/bin/activate
    python ~/gaitkit/scripts/run_parkinsonnw_benchmark.py \
        --data-dir ~/gait_benchmark_project/data/ParkinsonNW \
        --output   ~/gait_benchmark_project/results/benchmark_parkinsonnw.csv
...
import gaitkit
from gaitkit.extractors.parkinsonnw_extractor import ParkinsonNWExtractor
...
def run_benchmark(data_dir, methods, tolerances):
    ext = ParkinsonNWExtractor(data_dir)
    files = ext.list_files()
    print(f"Parkinson NW: found {len(files)} trials")
...
def main():
    parser = argparse.ArgumentParser(description="Run BIKE benchmark on Parkinson NW dataset")
    ...
    print("=" * 70)
    print("PARKINSON NW BENCHMARK (gaitkit detectors)")
    print("=" * 70)
    ...
    if args.output:
        ...
        out_path = Path("results") / f"benchmark_parkinsonnw_{ts}.csv"
