#!/usr/bin/env python3
"""
Vestibulopathy Benchmark — Run all gaitkit detectors on the vestibulopathy dataset.

Usage (on server):
    source ~/gait_venv/bin/activate
    python ~/gaitkit/scripts/run_vestibulopathy_benchmark.py \
        --data-dir ~/gait_benchmark_project/data/Vestibulopathy \
        --output   ~/gait_benchmark_project/results/benchmark_vestibulopathy.csv
...
import gaitkit
from gaitkit.extractors.vestibulopathy_extractor import VestibulopathyExtractor
...
def run_benchmark(data_dir, methods, tolerances):
    ext = VestibulopathyExtractor(data_dir)
    files = ext.list_files()
    print(f"Vestibulopathy: found {len(files)} trials")
    ...
    for fi, fpath in enumerate(files):
        ...
        population = file_info.get('population', 'unknown')
...
def main():
    parser = argparse.ArgumentParser(description="Run BIKE benchmark on Vestibulopathy dataset")
    ...
    print("=" * 70)
    print("VESTIBULOPATHY BENCHMARK (gaitkit detectors)")
    print("=" * 70)
    ...
    df = run_benchmark(args.data_dir, methods, tolerances)
    ...
    if args.output:
        ...
        out_path = Path("results") / f"benchmark_vestibulopathy_{ts}.csv"
