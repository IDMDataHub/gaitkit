#!/usr/bin/env python3
"""
Laroche Hip OA Benchmark — Run all gaitkit detectors on the Laroche hip OA dataset.

Usage (on server):
    source ~/gait_venv/bin/activate
    python ~/gaitkit/scripts/run_laroche_benchmark.py \
        --data-dir ~/gait_benchmark_project/data/LarocheHipOA \
        --output   ~/gait_benchmark_project/results/benchmark_laroche.csv
...
import gaitkit
from gaitkit.extractors.laroche_extractor import LarocheHipOAExtractor
...
DATASET_NAME = "laroche_hip_oa"


def match_events(detected, ground_truth, tolerance_frames):
...
def run_gaitkit_detect(method, angle_frames, fps, frame_offset=0):
    t0 = time.time()
    try:
...
def filter_max_trials(files, ext, max_per_subject_condition=5):
    """Keep at most N trials per (subject, condition) pair."""
    from collections import defaultdict
    counts = defaultdict(int)
    filtered = []
    for f in files:
        info = ext._parse_filename(f)
        key = (info['subject_id'], info['condition'])
        if counts[key] < max_per_subject_condition:
            filtered.append(f)
            counts[key] += 1
    return filtered


def run_benchmark(data_dir, methods, tolerances, max_trials=5):
    ext = LarocheHipOAExtractor(data_dir)
    files = ext.list_files()
    print(f"Laroche Hip OA: found {len(files)} total gait trials")

    if max_trials:
        files = filter_max_trials(files, ext, max_trials)
        print(f"  After filtering (max {max_trials}/subject/condition): {len(files)} trials")
    ...
    for fi, fpath in enumerate(files):
        ...
        # Determine population from filename
        file_info = ext._parse_filename(fpath)
        population = file_info.get('population', 'hip_oa')
    ...
    print(f"\nTotal: {len(df)} rows ({len(files)} trials x {len(methods)} detectors x {len(tolerances)} tolerances)")
    return df


def main():
    parser = argparse.ArgumentParser(description="Run BIKE benchmark on Laroche Hip OA dataset")
    ...
    parser.add_argument("--max-trials", type=int, default=5,
                        help="Max trials per subject per condition (default: 5, 0=all)")
    ...
    print("=" * 70)
    print("LAROCHE HIP OA BENCHMARK (gaitkit detectors)")
    print("=" * 70)
    ...
    df = run_benchmark(args.data_dir, methods, tolerances, max_trials=max_trials)
    ...
    if args.output:
        ...
        out_path = Path("results") / f"benchmark_laroche_{ts}.csv"
